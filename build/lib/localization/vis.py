...
def visualize_localization_3d(sfm_model, log, viewer):
    
    if not isinstance(sfm_model, pycolmap.Reconstruction):
        sfm_model = pycolmap.Reconstruction(sfm_model)
    
    # Draw Query and Retrieved Images poses and Visible 3D points       
    viewer.draw_localization(sfm_model, log)
    
            
def visualize_sfm_3d(sfm_model, viewer, n=500 , seed=2):
    
    if not isinstance(sfm_model, pycolmap.Reconstruction):
        sfm_model = pycolmap.Reconstruction(sfm_model)

    # Database Images poses and  3D points           
    viewer.draw_sfm(sfm_model)
