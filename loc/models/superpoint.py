...
from thirdparty.SuperPointPretrainedNetwork.demo_superpoint import SuperPointFrontend
...
class SuperPoint(nn.Module):
    ...
    """
    default_config = {
        'weights_path':         "thirdparty/SuperPointPretrainedNetwork/superpoint_v1.pth",
        'descriptor_dim':       256,
        'nms_radius':           4,
...
class SuperPointFeatures: 
    def __init__(self, do_cuda=True, logger=None): 
        ...
        if logger:
            logger.info("SuperPointFeature2D")
        ...
        # device
        if self.cuda:
            self.superpoint.cuda(self.device)
    ...
    def __call__(self, img, mask=None):  # mask is a fake input
        ...
        img  = img[None, None].to(self.device)
